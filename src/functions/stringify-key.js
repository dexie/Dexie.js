...
export function stringifyKey(key) {
  // key is IndexableType but declaring type here forces us to cast a lot.
...
export function unstringifyKey(str) {
  ...
  switch (str[0]) {
    ...
    case 'A':
      return JSON.parse(value).map(unstringifyKey);
    case 'B':
      return new Uint8Array(value.split(','));
    case 'D':
      return new Date(parseInt(value));
    default:
      throw new exceptions.InvalidArgument('Invalid key');
  }
}
