 ...
 *
 * Version 2.0.0-beta.8, Fri Jan 27 2017
 *
 * http://dexie.org
 *
...
var intrinsicTypes = "Boolean,String,Date,RegExp,Blob,File,FileList,ArrayBuffer,DataView,Uint8ClampedArray,ImageData,Map,Set"
    .split(',').concat(flatten([8, 16, 32, 64].map(num => ["Int", "Uint", "Float"].map(t => t + num + "Array")))).filter(t => _global[t]).map(t => _global[t]);
    ...
    get: function () {
        ...
        function then(onFulfilled, onRejected) {
            ...
            var rv = new Promise((resolve, reject) => {
                propagateToListener(this, new Listener(nativeAwaitCompatibleWrap(onFulfilled, psd, possibleAwait), nativeAwaitCompatibleWrap(onRejected, psd, possibleAwait), resolve, reject, psd));
            });
 ...
 * Apache License Version 2.0, January 2004, http://www.apache.org/licenses/
 */
var DEXIE_VERSION = '2.0.0-beta.8';
...
function Dexie(dbName, options) {
    ...
    var db = this;
    var dbReadyResolve, dbReadyPromise = new Promise(resolve => {
        dbReadyResolve = resolve;
    }), cancelOpen, openCanceller = new Promise((_, reject) => {
        cancelOpen = reject;
    });
    var autoSchema = true;
    ...
    */
    ...
    props(Table.prototype, {
        ...
        where: function (indexOrCrit) {
            ...
            // arbritary order:
            var compoundIndex = this.schema.indexes.concat(this.schema.primKey).filter(ix => ix.compound &&
            ...
            var simpleIndex = keyPaths.reduce((r, keyPath) => [
                ...
                r[0] || !idxByName[keyPath] ?
                    combine(r[1], x => '' + getByKeyPath(x, keyPath) ==
                        '' + indexOrCrit[keyPath])
                    : r[1]
            ], [null, null]);
            ...
            return this._idbstore(READWRITE, (resolve, reject, idbstore) => {
                ...
                const done = result => {
                    if (errorList.length === 0)
                        ...
                        reject(new BulkError(`${this.name}.bulkPut(): ${errorList.length} of ${numObjs} operations failed`, errorList));
                };
                var req, errorList = [], errorHandler, numObjs = objects.length, table = this;
        ...
        put: function (obj, key) {
            ...
            if (creatingHook !== nop || updatingHook !== nop) {
                ...
                obj = deepClone(obj);
                return this._trans(READWRITE, () => this.where(":id").equals(effectiveKey).modify(function () {
                    // Replace extisting value with our object
                    // CRUD event firing handled in Collection.modify()
                    this.value = obj;
                }).then(count => count === 0 ? this.add(obj, key) : effectiveKey), "locked"); // Lock needed because operation is splitted into modify() and add().
            }
            else {
