...
const DEXIE_VERSION = '4.0.1-alpha.11';
...
function applyOptimisticOps(result, req, ops, table, cacheEntry, immutable) {
        ...
        switch (op.type) {
            ...
            case 'deleteRange':
                ...
                modifedResult = result.filter((item) => {
                    ...
                    return !((rangesOverlap(new RangeSet(key), rangeSet) &&
    ...
    if (finalResult === result)
        return result;
    finalResult.sort((a, b) => cmp(sortIndex.extractKey(a), sortIndex.extractKey(b)));
    if (req.limit) {
...
function signalSubscribersNow(tblCache) {
    const collectedSubscribers = new Set();
                ...
                return {
                    ...
                    mutate(req) {
                        ...
                        if (primKey.autoIncrement && (req.type === 'add' || req.type === 'put') && (req.values.length < 50 || getEffectiveKeys(primKey, req).some(key => key == null))) {
                            ...
                            promise.catch(() => {
                                tblCache.optimisticOps.splice(tblCache.optimisticOps.indexOf(req), 1);
                                signalSubscribers(tblCache, req.mutatedParts);
                            });
                        }
                        return promise;
