 ...
 *
 * Version 3.1.0-alpha.1, Thu Nov 19 2020
 *
 * http://dexie.org
 *
...
var DEXIE_VERSION = '3.1.0-alpha.1';
...
var Collection =               (function () {
    ...
    Collection.prototype.modify = function (changes) {
        ...
        return this._write(function (trans) {
            ...
            var limit = _this.db._options.modifyChunkSize || 2000;
...
var Observable =               (function () {
    ...
    Observable.prototype.subscribe = function (next, error, complete) {
        return typeof next === "function" ?
            this._subscribe({ next: next, error: error, complete: complete }) :
...
function liveQuery(querier) {
    return new Observable(function (_a) {
        var start = _a.start, next = _a.next, error = _a.error;
        ...
        function shouldNotify() {
            for (var _i = 0, _a = keys(currentObs); _i < _a.length; _i++) {
                var db = _a[_i];
                var mutDb = accumMuts[db];
                if (mutDb) {
                    var obsDb = currentObs[db];
                    var _loop_1 = function (table) {
                        var mutTable = mutDb[table];
                        var obsTable = obsDb[table];
                        if (mutTable === true || obsTable === true)
                            return { value: true };
                        if (mutTable &&
                            mutTable.keys &&
                            obsTable.keys.some(function (key) {
                                return mutTable.keys.some(function (mKey) {
                                    try {
                    ...
                    for (var _b = 0, _c = keys(obsDb); _b < _c.length; _b++) {
                        var table = _c[_b];
...
function extendObservabilitySet(target, newSet) {
    keys(newSet).forEach(function (dbName) {
        var targetTableSet = target[dbName];
        var newTableSet = newSet[dbName];
        if (targetTableSet) {
            keys(newTableSet).forEach(function (tableName) {
                var targetPart = targetTableSet[tableName];
                var newPart = newTableSet[tableName];
...
var observabilityMiddleware = {
    ...
    create: function (core) {
        ...
        var getKey = function (req) { return [req.key]; };
        var getKeys = function (req) { return req.keys; };
        var getQueryKey = function (_a) {
            var _b = _a.query, index = _b.index, range = _b.range;
            return index.isPrimaryKey &&
                range.type === 1             && [range.lower];
        };
        var readSubscribers = [
            ["get", getKey],
            ["getMany", getKeys],
        ...
        return __assign(__assign({}, core), { table: function (tableName) {
                ...
                var tableClone = __assign(__assign({}, table), { mutate: function (req) {
                        var keys$$1 = req.type !== "deleteRange" &&
                            (req.type === "delete"
                                ? req.keys
                        ...
                        return table.mutate(req).then(function (res) {
                            var _a, _b;
                            var mutatedParts = (_a = {}, _a[dbName] = (_b = {}, _b[tableName] = keys$$1 && keys$$1.every(function (k) { return k != null; }) ? { keys: keys$$1 } : true, _b), _a);
                ...
                readSubscribers.forEach(function (_a) {
                    var method = _a[0], getKeys = _a[1];
                    tableClone[method] = function (req) {
                        var _a, _b;
                        if (PSD.subscr) {
                            var keys_1 = getKeys && getKeys(req);
                            var observabilitySet = (_a = {}, _a[dbName] = (_b = {}, _b[tableName] = keys_1
                                        ?
                                            { keys: keys_1, cmp: core.cmp }
                                        :
                                            true, _b), _a);
