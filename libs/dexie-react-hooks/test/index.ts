...
test('ItemLoaderComponent is reacting to changes', async () => {
  ...
  assert.equal(
    ...
    'NOT_FOUND: 1',
    'After deleting it - the component should say NOT_FOUND: 1'
  );
});
...
test('Selecting invalid key trigger the err-boundrary', async () => {
  ...
  await waitTilOk(
    () => /Something went wrong/.test(div.innerText),
    'The error boundrary should be shown'
  );
