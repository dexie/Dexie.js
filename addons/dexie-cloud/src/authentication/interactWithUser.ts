 ...
 * 
 * Security note: Remote SVG URLs are NOT fetched and inlined to prevent XSS.
 * Only built-in trusted icons are used as inline SVG. Remote icons are
 * rendered via <img> tags which are safe from script injection.
 */
function providerToOption(provider: OAuthProviderInfo): DXCOption {
  // Only use built-in trusted icons as inline SVG
  const iconSvg = ProviderIcons[provider.type];
  ...
  return {
    ...
    iconSvg,
    // If no built-in icon, use provider's iconUrl via img tag (safe)
