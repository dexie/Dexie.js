...
assert(
  shouldOffloadBlob(new Blob([new Uint8Array(1024)])),
  '1KB Blob is offloaded'
);
assert(
  shouldOffloadBlob(new Blob([new Uint8Array(8192)])),
  '8KB Blob is offloaded'
);

// File - always offloaded
assert(
  shouldOffloadBlob(new File(['x'], 'test.txt')),
  '1-byte File is offloaded'
);

// ArrayBuffer - threshold at 4096
assert(
  !shouldOffloadBlob(new ArrayBuffer(1)),
  '1-byte ArrayBuffer is NOT offloaded'
);
assert(
  !shouldOffloadBlob(new ArrayBuffer(4095)),
  '4095-byte ArrayBuffer is NOT offloaded'
);
assert(
  shouldOffloadBlob(new ArrayBuffer(4096)),
  '4096-byte ArrayBuffer IS offloaded'
);
assert(
  shouldOffloadBlob(new ArrayBuffer(8192)),
  '8KB ArrayBuffer IS offloaded'
);

// Uint8Array - threshold at 4096
assert(
  !shouldOffloadBlob(new Uint8Array(1)),
  '1-byte Uint8Array is NOT offloaded'
);
assert(
  shouldOffloadBlob(new Uint8Array(4096)),
  '4096-byte Uint8Array IS offloaded'
);
