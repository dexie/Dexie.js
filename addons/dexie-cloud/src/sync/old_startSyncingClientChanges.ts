import { DBCore, DBCoreRangeType, DBCoreSchema, DBCoreTable, DBCoreTableSchema } from "dexie";
import { Mut } from "../types/Mut";
import { TXExpandos } from "../types/TXExpandos";

export function startSyncingClientChanges(
  { mutReqs, txid }: IDBTransaction & TXExpandos,
  tables: string[],
  mutTableMap: Map<string, DBCoreTable>,
  core: DBCore,
  ordinaryTables: DBCoreTableSchema[]
) {
  /** TODO:
   * Open a new new transaction */
  const mutTables = [].slice.call(mutTableMap.values()) as DBCoreTable[];
  const mutTableReverseMap = new Map(
    tables.map((tableName) => [mutTableMap.get(tableName), tableName])
  );
  const trans = core.transaction({
    mode: "readonly",
    tables: [
      ...mutTables.map((tbl) => tbl.name),
      ...ordinaryTables.map(schema => schema.name)
    ],
  });
  return Promise.all(
    mutTables.map((tbl) =>
      tbl.query({
        trans,
        query: {
          index: tbl.schema.primaryKey,
          range: mutTableReverseMap.has(tbl)
            ? {
                type: DBCoreRangeType.Range,
                upper: mutReqs[mutTableReverseMap.get(tbl)!].firstRev,
                upperOpen: true,
                lower: 0,
                lowerOpen: true,
              }
    ...
    const responsesPerTable = mutTables.map((mutTable, idx) => ({
      mutTable,
      uncoveredMuts: responses[idx].result as Mut[],
    }));
