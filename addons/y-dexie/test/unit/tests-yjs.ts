...
promisedTest('Test DexieYProvider', async () => {
  ...
  deepEqual(
    doc!.getArray('arr').toJSON(),
    ['a', 'b', 'c'],
    'Array is correct after reload'
  );
  ...
  equal(
    updates.length,
    0,
    'No updates in update table after deleting document'
  );
});
...
promisedTest('Test Y document compression', async () => {
  ...
  equal(
    await db.table(updateTable).where('i').between(1, Infinity).count(),
    4,
    'Four updates stored after additional inserts'
  );
  console.debug('Running GC', await db.table(updateTable).toArray());
  await compressYDocs(db);
  console.debug('After running GC', await db.table(updateTable).toArray());
  ...
  console.debug(
    'After adding thigns to other doc',
    await db.table(updateTable).toArray()
  );
  equal(
    await db.table(updateTable).where('i').between(1, Infinity).count(),
    4,
    'Four updates stored after additional inserts on other doc'
  );
  await compressYDocs(db);
  console.debug(
    'After GC where we have 2 docs',
    await db.table(updateTable).toArray()
  );
  equal(
    await db.table(updateTable).where('i').between(1, Infinity).count(),
    2,
    'Two updates stored after gc (2 different docs)'
  );
...
promisedTest(
  'Test that syncers prohibit GC from compressing unsynced updates',
  async () => {
    await db.docs.put({
      id: 'doc1',
      title: 'Hello',
    ...
    ok(
      provider.destroyed,
      'Provider was destroyed when our document was deleted'
    );
  }
);
...
promisedTest('Verify error handling of misused Y.Doc properties', async () => {
  ...
  await db.docs
    ...
    .then(() => {
      ok(
        false,
        'Expected error when trying to create an Y.Doc property with null value'
      );
    })
    .catch((error) => {
      ok(
        error instanceof TypeError,
        'Expected TypeError when trying to create an Y.Doc property with null value'
      );
    });
  ...
  await db.docs
    ...
    .then(() => {
      ok(
        false,
        'Expected error when trying to create an Y.Doc property to a string'
      );
    })
    .catch((error) => {
      ok(
        error instanceof TypeError,
        'Expected TypeError when trying to create an Y.Doc property to a string'
      );
    });
  // Setting to a Blob:
  await db.docs
    ...
    .then(() => {
      ok(
        false,
        'Expected error when trying to create an Y.Doc property to a Blob'
      );
    })
    .catch((error) => {
      ok(
        error instanceof TypeError,
        'Expected TypeError when trying to create an Y.Doc property to a Blob'
      );
    });
  ...
  await db.docs
    ...
    .then(() => {
      ok(
        false,
        'Expected error when trying to put() with content data set on own property'
      );
    })
    .catch((error) => {
      ok(
        error instanceof TypeError,
        'Expected TypeError when trying to use put() with content data set on own property'
      );
    });
  ...
  equal(
    doc2Back!.title,
    'Quattro Pizza',
    'Verify that the put operation was successful for non-Y.Doc properties'
  );
  ok(
    doc2Back!.content === doc2!.content,
    "Verify that the Y.Doc property is equal be reference to the previously retrieved Y.Doc as long as it isn't destroyed"
  );
});
