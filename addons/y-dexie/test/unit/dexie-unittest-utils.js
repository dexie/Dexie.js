...
var no_optimize =
  window.no_optimize || window.location.search.indexOf('dontoptimize') !== -1;
...
function stringify(idbKey) {
  var res =
    '' +
    (idbKey && idbKey.constructor && idbKey.constructor === ArrayBuffer
      ? new Uint8Array(idbKey)
      : idbKey);
  return res;
}
                ...
                trans.storeNames
                  ...
                  .map(function (tableName) {
                    ...
                    return db.table(tableName).each(function (item, cursor) {
                      items[stringify(cursor.primaryKey)] = {
            ...
            .map(function (tableName) {
              ...
              return db
                ...
                .then(function () {
                  ...
                  return Promise.all(
                    Object.keys(initialItems).map((key) => {
                      var item = items[key];
                      var initialItem = initialItems[key];
                      if (
                        !item ||
                        JSON.stringify(item.value) !=
                          JSON.stringify(initialItem.value)
                      )
                        return db.table(tableName).schema.primKey.keyPath
                          ? db.table(tableName).put(initialItem.value)
                          : db
                              .table(tableName)
                              .put(initialItem.value, initialItem.key);
...
var hasPolyfillIE = [].slice
  .call(document.getElementsByTagName('script'))
  .some((s) => s.src.indexOf('idb-iegap') !== -1);
    ...
    switch (feature.toLowerCase()) {
      ...
      case 'binarykeys':
        try {
          return (
            result &&
            Array.isArray(Dexie.maxKey) &&
            indexedDB.cmp(new Uint8Array([1]), new Uint8Array([1])) === 0
          );
        } catch (e) {
          return false;
        }
