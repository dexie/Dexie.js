...
function Syncable(db) {
  ...
  if (
    ...
    (db.observable.version !== '{version}' &&
      !/^(3|4)\./.test(db.observable.version))
  )
    throw new Error(`Missing dexie-observable version 3.x or 4.x`);
  if (db.syncable) {
    if (db.syncable.version !== '{version}')
      throw new Error(
        `Mixed versions of dexie-syncable: "{version}" vs "${db.syncable.version}"`
      );
        ...
        Dexie.vip(() => {
          return db._syncNodes
            ...
            .toArray((connectedRemoteNodes) =>
              Promise.all(
                connectedRemoteNodes.map((node) =>
                  db.syncable
                    .connect(node.syncProtocol, node.url, node.syncOptions)
                    .catch((e) => {
                      console.warn(
                        `Dexie.Syncable: Could not connect to ${node.url}. ${e.stack || e}`
                      );
                    })
                )
              )
...
if (Dexie.Syncable) {
  if (Dexie.Syncable.version !== '{version}') {
    throw new Error(
      `Mixed versions of dexie-syncable: "{version}" vs "${Dexie.Syncable.version}"`
    );
  }
} else {
