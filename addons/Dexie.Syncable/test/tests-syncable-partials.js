...
(function () {
  ...
  asyncTest("client/server partials", function () {
    ...
    callbacks.push(function (context, url, options, baseRevision, syncedRevision, changes, partial, applyRemoteChanges, onChangesAccepted, onSuccess, onError) {
      applyRemoteChanges([{ type: 1, table: "objects", key: "apa", obj: { name: "uncommittedChange" } }], "revision with uncommitted", true, false)
          .then(() => {
          ...
          .then((changes) => {
            ok(changes.every(function (change) {return change.obj.name !== 'uncommittedChange'}), 'The uncommitted change should not be in _changes');
    ...
    callbacks.push(function (context, url, options, baseRevision, syncedRevision, changes, partial, applyRemoteChanges, onChangesAccepted, onSuccess, onError) {
      applyRemoteChanges([{ type: 1, table: "objects", key: "abba", obj: { name: "committedChange" } }], "revision one", false, false)
          .then(() => {
            return db1._uncommittedChanges.toArray().then((changes) => {
          ...
          .then((changes) => {
            ok(changes.some(function (change) { return change.obj.name === 'uncommittedChange' }), 'The uncommitted change should now be in _changes');
            ok(changes.some(function (change) { return change.obj.name === 'committedChange' }), 'The committedChange should also be in _changes');
          })
          .then(function () {
            return db1.delete();
          }).catch(function (err) {
