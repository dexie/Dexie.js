...
class MyDb extends Dexie {
  ...
  constructor() {
    ...
    //
    this.syncable
      .connect('myProtocol', 'https://remote-server/...', {
        anyOption: 'anyValue',
      })
      .catch((err) => {
...
db.syncable
  .getStatus('myUrl')
  .finally(() => {})
  .catch('DatabaseClosedError', (ex) => ex.name);
...
async function getUrlsAndStatuses() {
  ...
  let statuses = await Dexie.Promise.all(
    urls.map((url) => db.syncable.getStatus(url))
  );
}
