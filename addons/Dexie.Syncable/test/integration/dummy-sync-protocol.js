  ...
  sync: function (
    ...
    setTimeout(function () {
      ...
      onChangesAccepted().catch('DatabaseClosedError', function () {
        console.log(
          'Got DatabaseClosedError while calling onChangesAccepted()'
        );
      });

      applyRemoteChanges([], 'ServerRevision' + dummyRev++, true, false).catch(
        'DatabaseClosedError',
        function () {
          console.log(
            'Got DatabaseClosedError while calling applyRemoteChanges()'
          );
        }
      );
      applyRemoteChanges([], 'ServerRevision' + dummyRev++, false, false).catch(
        'DatabaseClosedError',
        function () {
          console.log(
            'Got DatabaseClosedError while calling applyRemoteChanges()'
    ...
    return function () {
      if (!db.dynamicallyOpened() && !db.connectAlreadyCalled) {
        ...
        db.syncable
          ...
          .then(function () {
            console.log(
              'connect() promise was resolved (database=' + db.name + ')'
            );
          })
          .catch('DatabaseClosedError', function () {})
          .catch(function (err) {
            console.error(
              'Error from connect() (database=' + db.name + '): ' + err.stack ||
                err
            );
          });
      }
