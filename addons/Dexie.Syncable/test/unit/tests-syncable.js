...
asyncTest('connect(), disconnect()', function () {
  ...
  var connectPromise = db1.syncable.connect(
    'testProtocol',
    'http://dummy.local',
    { option1: 'option1' }
  );
  ...
  callbacks.push(
    function (
      ...
      equal(
        changes.length,
        3,
        'Three changes (change number four should be reduced into change no 3'
      );
      ok(
      ...
      equal(
        partial,
        false,
        'Not partial since number of changes are below 1000'
      );
      // applyRemoteChanges
      applyRemoteChanges(
        [
    ...
    .then(function () {
      ...
      db1.objects.get('apa2', function (seven) {
        equal(
          seven.name,
          'seven',
          'Have got the change from the server. If not, check that promise does not fire until all changes have committed.'
        );
      });
    })
  ...
  callbacks.push(
    function (
      ...
      equal(
        baseRevision,
        'revision one',
        "First chunk of changes is based on revision one because 'six' was created based on revision one"
      );
      // syncedRevision
      equal(
        syncedRevision,
        'revision two',
        "Sync revision is 'revision two' because client has got it"
      );
      // changes
      equal(
        changes.length,
        1,
        "Even though there's two changes, we should only get the first one because they are based on different revisions"
      );
  ...
  callbacks.push(
    function (
      ...
      equal(
        baseRevision,
        'revision two',
        'Now we get changes based on revision two'
      );
      // syncedRevision
      equal(
        syncedRevision,
  ...
  callbacks.push(
    function (
      ...
      equal(
        syncedRevision,
        'revision two',
        'syncedRevision Still revision two'
      );
      // changes
      equal(changes.length, 2, 'Got 2 changes after reconnect.');
      equal(
        changes[0].obj.name,
        'changeAfterDisconnect',
        'change one is changeAfterDisconnect'
      );
      equal(
        changes[1].obj.name,
        'changeBeforeReconnect',
        'change two is changeBeforeReconnect'
      );
      onChangesAccepted();
      ...
      setTimeout(function () {
        db1.syncable
          .getStatus('http://dummy.local', function (status) {
    ...
    function (
      ...
      applyRemoteChanges(
        ...
        .then((uncommittedChanges) => {
          ok(
            uncommittedChanges.every(function (change) {
        ...
        .then((nodes) => {
          strictEqual(
            nodes.length,
            0,
            'All nodes with this url should be deleted'
          );
          ok(
            deleteOldChangesWasCalled,
            'Observable.deleteOldChanges was called'
          );
        })
        .catch(function (err) {
          ok(false, 'Got error: ' + err);
