...
asyncTest('should return a new node if none exists for the given URL', () => {
  ...
  getOrCreateSyncNode(nodeOpts)
    .then((node) => {
      nodeID = node.id;
      ok(
        node instanceof db.observable.SyncNode,
        'returned node is instance of SyncNode'
      );
      strictEqual(
        node.syncProtocol,
        protocolName,
        'syncProtocol is protocol name'
      );
      strictEqual(node.url, url, 'url is the url we passed to init');
      deepEqual(
        node.syncOptions,
        nodeOpts,
        'syncOptions are the same as the options we passed'
      );
      strictEqual(node.myRevision, -1, 'myRevision is -1');
      propEqual(
        node.syncContext,
        { nodeID },
        'syncContext contains the correct nodeID'
      );
...
asyncTest(
  'should return an existing node if one exists for the given URL',
  () => {
    const nodeOpts = { foo: 'bar' };
    let nodeID;
    ...
    db._syncNodes
      ...
      .then((node) => {
        ...
        propEqual(
          node.syncOptions,
          nodeOpts,
          'node contains the given options'
        );
        return db._syncNodes.get(addedNodeID);
      })
      .then((node) => {
        strictEqual(
          node.syncContext.nodeID,
          addedNodeID,
          'Node was saved in the DB with the correct context'
        );
      })
      .catch(function (err) {
        ok(false, 'Error: ' + err);
...
asyncTest(
  'should set myRevision to the last _changes if initialUpload is false',
  () => {
    ...
    db._changes
      .add({ key: 1, obj: { foo: 'bar' }, table: 'foo', type: 1 })
      .then(() => {
        return getOrCreateSyncNode(nodeOpts);
      })
      .then((node) => {
