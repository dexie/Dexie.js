...
asyncTest(
  'should apply _uncommittedChanges and remove them from that table',
  () => {
    const createChange = {
      key: 1,
      node: nodeID,
    ...
    db._uncommittedChanges
      ...
      .then((count) => {
        strictEqual(
          count,
          0,
          'No more entries in _uncommittedChanges for the given nodeID'
        );
      })
      .catch(function (err) {
        ok(false, 'Error: ' + err);
...
asyncTest(
  'should update remoteBaseRevisions and remove any old revisions',
  () => {
    const createChange = {
      key: 1,
      node: nodeID,
    ...
    finallyCommitAllChanges([createChange], remoteRevision)
      .then(() => {
        strictEqual(
          syncNode.remoteBaseRevisions.length,
          1,
          'Only one remoteBaseRevision'
        );
        // We had no changes yet so the next local revision is 1
        deepEqual(
          syncNode.remoteBaseRevisions,
          [{ local: 1, remote: remoteRevision }],
          'Make sure remoteBaseRevisions contains the correct object'
        );
      })
      .catch(function (err) {
        ok(false, 'Error: ' + err);
