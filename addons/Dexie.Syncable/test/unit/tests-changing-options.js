...
asyncTest('Change options on an existing node', function () {
  ...
  db.syncable
    .connect(protocolName, serverUrl, { option1: 'option1' })
    .then(() => {
    ...
    .then((options) => {
      propEqual(
        options,
        { option1: 'option1' },
        'getOptions got the correct options'
      );
      ...
      syncProtocol.sync = function (
        ...
        propEqual(
          options,
          { newOptions: 'other options' },
          'sync got the new options'
        );
        onSuccess({ again: 1000 });
      };
      ...
      return db.syncable.connect(protocolName, serverUrl, {
        newOptions: 'other options',
      });
    })
    .then(() => {
    ...
    .then((options) => {
      propEqual(
        options,
        { newOptions: 'other options' },
        'getOptions got the new options'
      );
    ...
    .then(() => {
      syncProtocol.sync = function (
        ...
        propEqual(
          options,
          { evenNewerOptions: 'super new options' },
          'sync got the even newer options'
        );
        onSuccess({ again: 1000 });
      };
    ...
    .then((options) => {
      propEqual(
        options,
        { evenNewerOptions: 'super new options' },
        'getOptions got the even newer options'
      );
    })
    .catch(function (err) {
      ok(false, 'Error: ' + err);
