...
asyncTest(
  'should get the contents of our tables and create CREATE changes if node.myRevision is -1',
  () => {
    ...
    function cb(
      changes /*, remoteBaseRevision, partial, nodeModificationsOnAck*/
    ) {
      ...
      deepEqual(
        hasMoreToGive,
        { hasMoreToGive: false },
        "it should't change hasMoreToGive"
      );
    }
    db.foo
      .add(fooTableObject)
...
asyncTest('should return changes in _changes if myRevision >= 0', () => {
  ...
  db.foo
    .bulkAdd([fooTableObject1, fooTableObject2])
    .then(() => {
      return getLocalChangesForNode(syncNode, cb);
    })
    .catch(function (err) {
...
asyncTest(
  'should return partial changes in _changes if myRevision >= 0 and threshold was reached',
  () => {
    syncNode.myRevision = -1;
    syncNode.dbUploadState = null;
    syncNode.remoteBaseRevisions = [];
    ...
    function cb(
      ...
      deepEqual(
        hasMoreToGive,
        { hasMoreToGive: true },
        'it should change hasMoreToGive'
      );
    }
    // This also adds changes to _changes
    db.foo
...
asyncTest(
  'should not return any changed if the threshold is 0 but should set hasMoreToGive to true',
  () => {
    syncNode.myRevision = -1;
    syncNode.dbUploadState = null;
    syncNode.remoteBaseRevisions = [];
