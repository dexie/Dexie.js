...
asyncTest('client/server partials', function () {
  ...
  callbacks.push(
    function (
      ...
      // applyRemoteChanges
      applyRemoteChanges([], 'revision one', false, false)
        .then(function () {
          // onChangesAccepted
          onChangesAccepted();
        })
  ...
  callbacks.push(
    function (
      ...
      onChangesAccepted();
      db1
        .transaction('rw', db1.objects, function () {
  ...
  callbacks.push(
    function (
      ...
      equal(
        changes.length,
        partialsThreshold,
        `Got ${partialsThreshold} changes`
      );
      equal(changes[0].obj.name, 'bulk', 'change is bulk');
      equal(
        partial,
        true,
        `More than ${partialsThreshold} changes gives partial=true`
      );
      onChangesAccepted();
      onSuccess({ again: 1 });
    }
    ...
    function (
      ...
      applyRemoteChanges(
        ...
        .then(() => {
          return db1._uncommittedChanges.toArray().then((changes) => {
            strictEqual(
              changes.length,
              1,
              'Should have one uncommitted change'
            );
            strictEqual(changes[0].key, 'apa', 'Key should match');
            deepEqual(
              changes[0].obj,
              { name: 'uncommittedChange' },
              'Saved obj should match'
            );
        ...
        .then((changes) => {
          ...
          return db1._syncNodes
            .where('url')
            .equals('http://dummy.local')
            .toArray();
        })
        .then((nodes) => {
          strictEqual(
            nodes[0].appliedRemoteRevision,
            'revision with uncommitted',
            "The node's appliedRemoteRevision should be updated"
          );
          onSuccess({ again: 1 });
        });
    }
