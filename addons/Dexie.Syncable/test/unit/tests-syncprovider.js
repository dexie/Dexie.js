...
module('tests-syncprovider', {
  setup: function () {
    stop();
    Dexie.Promise.all(
      Dexie.delete('SyncProviderTest'),
      Dexie.delete('OtherClientDB')
    )
      .then(function () {
        start();
      })
...
asyncTest('testSyncProvider', function () {
  ...
  waitFor(db2, { type: CREATE, name: 'David' })
    .then(function () {
      ok(
        true,
        "The CREATE of friend 'David' was sent all the way to server and then back again to db2."
      );
    ...
    .then(function () {
      ok(
        true,
        "The UPDATE of friend 'David' to 'Ylva' was sent all the way around as well"
      );
      return db.friends
        .where('name')
        .equals('Ylva')
        .first(function (friend) {
          return friend.id;
        });
    })
    .then(function (id) {
    ...
    .then(function () {
      ok(
        true,
        "The DELETE of friend 'Ylva' was sent all the way around as well"
      );
    ...
    .then(function () {
      ok(
        true,
        'Successfully added 1100 pets into db2. Now wait for the last change to be synced into db1.'
      );
    ...
    .then(function () {
      ok(
        true,
        'All 1100 dogs where sent all the way around (db2-->db this time)'
      );
    ...
    .then(function () {
      ok(
        true,
        'Successfully added 1000 cats. Now wait for them to arrive in db2.'
      );
    ...
    .then(function () {
      ok(
        true,
        'All 1000 cats where sent all the way around (db-->db2 this time)'
      );
    })
    .finally(function () {
