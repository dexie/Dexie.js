...
(function () {
  ...
  asyncTest('delete()', () => {
    ...
    callbacks.push(function (context, url, options, baseRevision, syncedRevision, changes, partial, applyRemoteChanges, onChangesAccepted, onSuccess, onError) {
      ...
      applyRemoteChanges([{ type: 1, table: "objects", key: "apa", obj: { name: "uncommittedChangeBeforeDelete" } }], "revision with uncommitted", true, false)
          .then(() => {
          ...
          .then((uncommittedChanges) => {
            ok(uncommittedChanges.every(function (change) {return change.obj.name !== 'uncommittedChangeBeforeDelete'}), 'The uncommitted change should not be in _uncommittedChanges anymore');
