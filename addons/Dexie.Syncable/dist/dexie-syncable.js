 ...
 * ==========================================================================
 *
 * Version 4.0.1-beta.13, Sun May 26 2024
 *
 * https://dexie.org
 *
...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, (global.Dexie = global.Dexie || {}, global.Dexie.Syncable = factory(global.Dexie)));
    ...
    var Promise$3 = Dexie__default["default"].Promise;
    function initSyncableConnect(db, connect) {
        return function syncableConnect(protocolInstance, protocolName, url, options) {
            if (db.isOpen()) {
                ...
                return Promise$3.reject(new Dexie__default["default"].DatabaseClosedError());
            }
            else if (db.hasFailed()) {
                // Database has failed to open
                return Promise$3.reject(new Dexie__default["default"].InvalidStateError("Dexie.Syncable: Cannot connect. Database has failed to open"));
            }
            else {
                ...
                var promise = new Promise$3(function (resolve, reject) {
                    ...
                    db.open().catch(function (ex) {
                        ...
                        reject(new Dexie__default["default"].InvalidStateError("Dexie.Syncable: Couldn't connect. Database failed to open", ex));
                    });
                });
                return promise;
            }
            ...
            return db.transaction('rw', db._syncNodes, db._changes, function () {
                ...
                return db._syncNodes.where("url").equalsIgnoreCase(url).first(function (node) {
                    ...
                    if (node) {
                        ...
                        Dexie__default["default"].Promise.resolve(function () {
        ...
        return function enqueue(context, fn, instanceID) {
            function _enqueue() {
                if (!context.ongoingOperation) {
                    context.ongoingOperation = Dexie__default["default"].ignoreTransaction(function () {
                        return Dexie__default["default"].vip(function () {
                            return fn();
                        });
                    }).finally(function () {
            ...
            if (!instanceID) {
                ...
                if (db.isOpen()) {
                    ...
                    return Dexie__default["default"].Promise.reject(new Dexie__default["default"].DatabaseClosedError());
                }
            }
        ...
        return table.where(':id').anyOf(keys).raw().each(function (obj, cursor) {
            ...
            var objsToPut = updatesThatApply.map(function (c) {
                ...
                Object.keys(c.mods).forEach(function (keyPath) {
                    Dexie__default["default"].setByKeyPath(curr, keyPath, c.mods[keyPath]);
                });
                return curr;
            });
    ...
    function combineCreateAndUpdate(prevChange, nextChange) {
        var clonedChange = Dexie__default["default"].deepClone(prevChange); // Clone object before modifying since the earlier change in db.changes[] would otherwise be altered.
        Object.keys(nextChange.mods).forEach(function (keyPath) {
            Dexie__default["default"].setByKeyPath(clonedChange.obj, keyPath, nextChange.mods[keyPath]);
        });
        return clonedChange;
    }
    ...
    function combineUpdateAndUpdate(prevChange, nextChange) {
        ...
        Object.keys(nextChange.mods).forEach(function (keyPath) {
            ...
            Object.keys(prevChange.mods).filter(function (parentPath) { return keyPath.indexOf(parentPath + '.') === 0; }).forEach(function (parentPath) {
                Dexie__default["default"].setByKeyPath(clonedChange.mods[parentPath], keyPath.substr(parentPath.length + 1), nextChange.mods[keyPath]);
                hadParentPath = true;
            });
            if (!hadParentPath) {
            ...
            if (node.myRevision >= 0) {
                ...
                if (node.dbUploadState === null) {
                    ...
                    if (tablesToUpload.length === 0)
                        return Dexie__default["default"].Promise.resolve(cb([], null, false, {})); // There are no synced tables at all.
                    var dbUploadState = {
                    ...
                    return getTableObjectsAsChanges(Dexie__default["default"].deepClone(node.dbUploadState), [], collection);
                }
                else {
                    var collection = db.table(dbUploadState.currentTable).orderBy(':id');
                    return getTableObjectsAsChanges(Dexie__default["default"].deepClone(node.dbUploadState), [], collection);
        ...
        return function connectProtocol(node, activePeer) {
            ...
            function sendChangesToProvider(changes, remoteBaseRevision, partial, nodeModificationsOnAck) {
                ...
                var finalSyncPromise = new Promise$2(function (resolve, reject) {
                    ...
                    Dexie__default["default"].asap(function () {
                        try {
                ...
                function onChangesAccepted() {
                    Object.keys(nodeModificationsOnAck).forEach(function (keyPath) {
                        Dexie__default["default"].setByKeyPath(node, keyPath, nodeModificationsOnAck[keyPath]);
                    });
            ...
            function applyRemoteChanges(remoteChanges, remoteRevision, partial /*, clear*/) {
                ...
                return enqueue(applyRemoteChanges, function () {
                    if (!stillAlive())
                        return Promise$2.reject(new Dexie__default["default"].DatabaseClosedError());
    ...
    function initConnectFn(db, activePeers) {
        return function connect(protocolInstance, protocolName, url, options, dbAliveID) {
            ...
            if (existingPeer.length > 0) {
                ...
                var diffObject = {};
                Dexie__default["default"].getObjectDiff(activePeer.syncOptions, options, diffObject);
            ...
            function execConnect() {
                ...
                var activePeer = {
                    ...
                    syncOptions: options,
                    on: Dexie__default["default"].Events(null, "disconnect"),
     ...
     *
     */
    var override = Dexie__default["default"].override, Promise$1 = Dexie__default["default"].Promise, Observable = Dexie__default["default"].Observable;
    /** Dexie addon for remote database sync.
     *
     * @param {Dexie} db
     */
    function Syncable(db) {
        if (!/^(3|4)\./.test(Dexie__default["default"].version))
            throw new Error("Missing dexie version 3.x or 4.x");
        if (!db.observable || (db.observable.version !== "4.0.1-beta.13" && !/^(3|4)\./.test(db.observable.version)))
            throw new Error("Missing dexie-observable version 3.x or 4.x");
        if (db.syncable) {
            if (db.syncable.version !== "4.0.1-beta.13")
                throw new Error("Mixed versions of dexie-syncable: \"4.0.1-beta.13\" vs \"".concat(db.syncable.version, "\""));
        ...
        db.on('message', function (msg) {
            ...
            Dexie__default["default"].vip(function () {
                if (msg.type === 'connect') {
        ...
        db.on('cleanup', function (weBecameMaster) {
            ...
            if (weBecameMaster) {
                ...
                // an old closed one. 
                Dexie__default["default"].ignoreTransaction(function () { return Dexie__default["default"].vip(function () {
        ...
        db.syncable.on = Dexie__default["default"].Events(db, { statusChanged: "asap" });
        db.syncable.disconnect = function (url) {
            return Dexie__default["default"].ignoreTransaction(function () {
    ...
    Syncable.version = "4.0.1-beta.13";
    ...
    if (Dexie__default["default"].Syncable) {
        if (Dexie__default["default"].Syncable.version !== "4.0.1-beta.13") {
            throw new Error("Mixed versions of dexie-syncable: \"4.0.1-beta.13\" vs \"".concat(Dexie__default["default"].Syncable.version, "\""));
        }
    }
    else {
        // Register addon in Dexie:
        Dexie__default["default"].Syncable = Syncable;
        Dexie__default["default"].addons.push(Syncable);
    }
    var Dexie_Syncable = Dexie__default["default"].Syncable;
