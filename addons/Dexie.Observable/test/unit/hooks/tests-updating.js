...
const wakeupObservers = initWakeupObservers(
  db,
  { latestRevision: {} },
  self.localStorage
);
      ...
      try {
        db._changes
          .toArray((changes) => {
            ...
            ok(
              typeof trans._lastWrittenRevision !== 'undefined',
              '_lastWrittenRevision should be defined'
            );
          })
          .catch((e) => {
            ok(false, 'Error: ' + e);
...
asyncTest(
  'should not create an UPDATE change if the mods are already in the old object',
  () => {
    db.foo.schema.observable = true;
    const updatingHook = initUpdatingHook(db, db.foo.name);
    const ID = 2;
