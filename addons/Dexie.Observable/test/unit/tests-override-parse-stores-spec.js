...
test('should create stores for Dexie.Observable and Dexie.Syncable', () => {
  function origFunc(stores /*, dbSchema*/) {
    ...
    ok(
      typeof stores._uncommittedChanges === 'string',
      'Should have _uncommittedChanges store'
    );
  }
...
test('should add UUID keys to the schema', () => {
  ...
  strictEqual(
    dbSchema.foo.primKey.name,
    'id',
    'Should remove $$ from the name'
  );
  strictEqual(
    dbSchema.foo.primKey.keyPath,
    'id',
    'Should remove $$ from keyPath'
  );
...
test('should observe tables without _ and $', () => {
  ...
  strictEqual(
    dbSchema._foo.observable,
    undefined,
    '_foo should not be observed'
  );
  strictEqual(
    dbSchema.$bar.observable,
    undefined,
    '$bar should not be observed'
  );
});
