...
asyncTest(
  'changes in on DB instance should trigger a change event in an other instance',
  4,
  function () {
    var db1 = createDB();
    var db2 = createDB();
        ...
        switch (change.type) {
          ...
          case 2:
            ...
            equal(
              ...
              JSON.stringify({ name: 'David' }),
              'Only modifying the name property'
            );
            break;
          case 3:
...
asyncTest('Capital$$Id-test', function () {
  var db = createDB();
  db.open();
  db.CapitalIdTest.put({ name: 'Hilda' })
    .then(function () {
...
asyncTest('should receive a message from the first sync node', 4, () => {
  ...
  db2.on('message', (msg) => {
    ...
    strictEqual(
      msg.destinationNode,
      receiverID,
      'We got the correct destination node'
    );
    start();
  });
...
asyncTest(
  'master node should receive the message if the destination is not present and we want a reply',
  5,
  () => {
    const db1 = createDB();
    let senderID;
    ...
    db1._syncNodes
      ...
      .then((arr) => {
        ...
        db1.observable.sendMessage('request', { foo: 'foobar' }, 10, {
          wantReply: true,
        });
      })
      .catch((e) => {
...
asyncTest('sender should react on successful reply', 1, () => {
  ...
  db1._syncNodes
    ...
    .then((arr) => {
      receiverID = arr.filter((node) => node.id !== senderID)[0].id;
      return db1.observable.sendMessage(
        'request',
        { foo: 'foobar' },
        receiverID,
        { wantReply: true }
...
asyncTest(
  'sender should be able to react on broadcast if bIncludeSelf is true',
  4,
  () => {
    const db1 = createDB();
    const db2 = createDB();
    ...
    db1._syncNodes
      ...
      .then((arr) => {
        ...
        db1.observable.broadcastMessage(
          'broadcast',
          { foo: 'foobar' },
          bIncludeSelf
        );
...
asyncTest(
  'no matter how many times sendMessage is called a receiver should receive its message only once',
  4,
  () => {
    const db1 = createDB();
    const db2 = createDB();
