...
promisedTest('export-format', async () => {
  ...
  const outboundKeys = await importedDB
    .table('outbound')
    .toCollection()
    .primaryKeys();
  ...
  ok(
    'getTime' in (outboundKeys[1] as Date),
    'Second outbound key should be a Date instance'
  );
  equal(
    (outboundKeys[1] as Date).getTime(),
    1,
    "The time '1' should be the value of the Date key"
  );
  ...
  deepEqual(
    [].slice.call(ba),
    [].slice.call(fullByteArray),
    'The whole byte spectrum supported after redecoding blob'
  );
...
promisedTest('unicode-roundtrip-no-corruption', async () => {
  ...
  equal(
    corrupted,
    0,
    'No rows should contain replacement characters (U+FFFD) after import'
  );
  importedDB.close();
  await Dexie.delete(UNICODE_DB);
});
