(function (global, factory) {
    ...
    ***************************************************************************** */
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    ...
                    QUnit.ok(generatedKey.privateKey instanceof CryptoKey, "The CryptoKey object was not generated correctly");
                    return [4 /*yield*/, db.keys.add(objToAdd)];
                case 2:
                    id = _b.sent();
                    QUnit.ok(id != null, "Got unexpectedly nullish id");
                    return [4 /*yield*/, db.keys.get(id)];
                case 3:
                    storedObj = _b.sent();
                    QUnit.ok(storedObj.key instanceof CryptoKey, "The CryptoKey object was destroyed in storage");
                    return [2 /*return*/];
            }
        });
    });
    ...
    db$a.version(1).stores({ users: "id,first,last,&username,*&email,*pets" });
    ...
    ***************************************************************************** */
    ...
    var mutsAndExpects = function () { return [
        ...
        [
            ...
            }); },
            {
                friendsOver18: __spreadArray([
                    { get id() { return lastFriendId; }, name: "Foo", age: 20 },
                    { get id() { return barbarFriendId; }, name: "Barbar", age: 21 }
                ], __read(bulkFriends.map(function (f) { return ({ name: f.name, age: f.age, get id() { return f.id; } }); }).filter(function (f) { return f.age > 18; }))).sort(function (a, b) { return a.age - b.age; })
            }
        ],
        ...
        [
            ...
            {
                outboundToArray: __spreadArray([{ num: 1, name: "A" }, { num: 2, name: "B" }, { num: 3, name: "C" }], __read(bulkOutbounds)),
                outbound_above_z49: __spreadArray([], __read(bulkOutbounds.filter(function (o) { return o.name > "z49"; })))
            },
        ...
        [
            ...
            {
                friendsOver18: __spreadArray([], __read(bulkFriends.filter(function (f) { return f.age > 18; })))
            }
        ],
        // bulkDelete
        [
            ...
            {
                friendsOver18: __spreadArray([], __read(bulkFriends.filter(function (f) { return f.age === 20; })))
            }
        ],
        // multiEntry
        [
    ...
    QUnit.test("it should balance itself", function () {
        ...
        QUnit.equal(__spreadArray([], __read(set)).length, 1, "After adding the super range, the entire set should just contain the super set");
    });
    QUnit.test("it should be iterable", function () {
        ...
        QUnit.equal(__spreadArray([], __read(set)).length, 10, "Should iterate 10 items");
        QUnit.ok(__spreadArray([], __read(set)).every(function (node, i) { return node.from === i && node.to === i; }), "Each node should have correct ranges");
    });
    QUnit.test("it should be mergable", function () {
        var set = RangeSet().addKeys([0, 2, 4, 7, 8]);
        var set2 = RangeSet().addKeys([1, 3, 5, 7, 9, 11]);
        set.add(set2);
        QUnit.equal(__spreadArray([], __read(set)).map(function (_a) {
            var from = _a.from;
            return from;
        }).join(','), [
            0,
        ...
        QUnit.equal(JSON.stringify(__spreadArray([], __read(set)).map(function (_a) {
    ...
    function isSequencial(set) {
        ...
        try {
            for (var _b = __values(__spreadArray([], __read(set))), _c = _b.next(); !_c.done; _c = _b.next()) {
                var node = _c.value;
    ...
    QUnit.test("stress", function () {
        ...
        //console.log("depth", set.d);
        QUnit.equal(__spreadArray([], __read(set)).length, 600, "Set should contain individual ranges");
        QUnit.ok(isSequencial(set), "set is sequencial");
        //debugger;
        ...
        QUnit.ok(isSequencial(set), "set is sequencial");
        //console.log("depth", set.d);
        QUnit.equal(__spreadArray([], __read(set)).length, 559, "Set should have less ranges");
        //console.log("depth", set.d);
    });
