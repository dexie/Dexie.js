...
db.on('populate', () => {
  db.items.bulkAdd([{ id: 1 }, { id: 2 }, { id: 3 }]);
  db.outbound.bulkAdd(
    [
      { num: 1, name: 'A' },
      { num: 2, name: 'B' },
...
promisedTest('subscribe to range', async () => {
  ...
  await tester.waitTilDeepEqual(
    [{ id: 1 }, { id: 2 }, { id: 3 }],
    'First callback should give initally populated content'
  );
  db.items.add({ id: -1 });
  await tester.waitTilDeepEqual(
    [{ id: -1 }, { id: 1 }, { id: 2 }, { id: 3 }],
    '2nd callback should give updated content'
  );
  db.items.delete(2);
  await tester.waitTilDeepEqual(
    [{ id: -1 }, { id: 1 }, { id: 3 }],
    '3rd callback should wake up when deletion was made'
  );
});
...
promisedTest('subscribe to keys', async () => {
  ...
  ok(
    res1 === undefined,
    'The updated result of db.items.get(1) should return undefined after the deletion'
  );
  ...
  ok(
    res2 === undefined,
    'The updated result of db.items.get(2) should return undefined after the deletion'
  );
  ...
  equal(
    count1,
    3,
    'Just double-checking - no more calls after having unsubscribed'
  );
});
...
promisedTest('subscribe and error occur', async () => {
  ...
  equal(
    result,
    'error',
    "The observable's error callback should have been called"
  );
...
promisedTest(
  'optimistic updates that eventually fail must be reverted (Issue #1823)',
  async () => {
    const log = [];
    let subscription = liveQuery(() => db.items.toArray()).subscribe({
      next: (result) => {
        log.push(result);
    ...
    deepEqual(
      log.at(-1),
      [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 99, iWillSucceed: true }],
      'Last log entry contains the correct result. There might be optimistic updates before though.'
    );
  }
);
...
const mutsAndExpects = () => [
  ...
  [
    () =>
      db.friends
        .put({ name: 'Barbar', age: 21 })
        .then((id) => (barbarFriendId = id)),
    {
      friendsOver18: [
        {
          get id() {
  ...
  [
    // Delete all but one:
    () =>
      db.friends.bulkDelete(
        bulkFriends.filter((f) => f.age !== 20).map((f) => f.id)
      ),
    {
...
promisedTest('Full use case matrix', async () => {
  const queries = {
    ...
    itemsStartsWithAPrimKeys: () =>
      db.items.where('name').startsWith('A').primaryKeys(),
    itemsStartsWithAOffset3: () =>
      db.items.where('name').startsWith('A').offset(3).toArray(),
    ...
    outboundStartsWithA: () =>
      db.outbound.where('name').startsWith('A').toArray(),
    outboundIdBtwnMinus1And2: () =>
      db.outbound.where(':id').between(-1, 2, true, true).toArray(),
    outboundAnyOf_BCD_keys: () =>
      db.outbound.where('name').anyOf('B', 'C', 'D').keys(),
    outbound_above_z49: () => db.outbound.where('name').above('z49').toArray(),

    friendsOver18: () => db.friends.where('age').above(18).toArray(),

    multiEntry1: () =>
      db.multiEntry.where('tags').startsWith('A').primaryKeys(),
    ...
    compoundOrderByWithAutoIncKey: () =>
      db.issue1946.orderBy('[name+age+id]').toArray(),
  };
    ...
    Object.keys(queries).map((name) => [
      ...
      liveQuery(async () => {
        ...
        try {
          ...
          console.log(
            `Setting actual result of ${name} to ${JSON.stringify(res)}`
          );
  ...
  function zeroFlyers(abortSignal) {
    return new Promise((resolve, reject) => {
      ...
      abortSignal.addEventListener(
        'abort',
        () => reject(new Error('flyers timeout')),
        { once: true }
      );
    });
  }
...
promisedTest('RxJS compability', async () => {
  let signal = new Signal();
  const o = from(liveQuery(() => db.items.toArray())).pipe(
...
promisedTest(
  'Isolation: Explicit rw transactions do not affect live queries before committed',
  async () => {
    let log = [];
    let signal = new Signal();
    let subscription = liveQuery(() => db.items.toArray()).subscribe(
    ...
    deepEqual(
      ...
      [
        ...
        {
          type: 'emit',
          result: [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4, name: 'A' }],
        },
      ],
    ...
    //signal = new Signal();
    await db
      .transaction('rw', db.items, async (tx) => {
...
promisedTest(
  'Issue 1821: liveQuery containing primaryKeys should not emit when content on one element is changed',
  async () => {
    let log = [];
    let signal = new Signal();
    let subscription = liveQuery(() =>
      db.items.orderBy('id').primaryKeys()
    ).subscribe((result) => {
      log.push({ type: 'emit', result });
      signal.resolve(result);
    });
...
promisedTest(
  'Issue 2067: useLiveQuery does not update when multiple items are deleted',
  async () => {
    ...
    await tester.waitTilDeepEqual(
      [{ id: 0 }, { id: -10 }],
      'Should have deleted items where id > 0'
    );
  }
);

promisedTest(
  'Issue 2058 - related but with bulkAdd and constraint error on duplicate primary keys.',
  async () => {
    ...
    await db.items
      ...
      .catch((error) => {
        equal(
          error.failuresByPos[0].name,
          'ConstraintError',
          'Expected constraint error for the first operation'
        );
      });
    await tester.waitTilDeepEqual(
      [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 88 }],
    ...
    await db.items.bulkAdd(itemsToAdd).catch((error) => {
      deepEqual(
        Object.keys(error.failuresByPos).map(Number),
        [
...
promisedTest('Issue 2058: Cache error after bulkPut failures', async () => {
  const tester = liveQueryUnitTester(() => db.issue2058.toArray(), {
    graceTime: 10,
  });
