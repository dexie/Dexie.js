...
const { stop, start, asyncTest, test, equal, notEqual, deepEqual, ok } = require("qunitjs"); // eslint-disable-line no-unused-vars
const {
    promisedTest,
    spawnedTest,
    supports,
    resetDatabase
} = require("./dexie-unittest-utils-node.js"); // eslint-disable-line no-unused-vars

// Monkey patch the test of IndexedDB and promises (node has promises and we just included the IndexedDBShim).
async function isIdbAndPromiseCompatible() {
